...
//Search Google kor a keyword and extract ranking results for a target domain
...
export async function rankTracker(keyword, targetDomain) {
    ...
    try {
        ...
        //Inital Google visit and consent handling
        ...
        for (let gPage = 0; gPage < 5; gPage++) {
            ...
            for (let retry = 0; retry < 3; retry++) {
                try {
                    ...
                    await page.waitForTimeout(1500);
                    pageResults = await page.evaluate(() => Array.from(document.querySelectorAll("h3"))
                        .map((h3) => {
                            let a = h3.closest('a');
                            if (!a) {
            ...
            for (const r of pageResults) {
                ...
                allResults.push(r);
                if (!found && (r.domain.includes(cleanTarget) || cleanTarget.includes(r.domain.toLowerCase()))) {
